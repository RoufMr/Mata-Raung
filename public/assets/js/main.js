...
if ($(".photo-popup").length) {
    $(".photo-popup").magnificPopup({
        type: "image", // Ganti dari "iframe" ke "image"
        mainClass: "mfp-fade",
        removalDelay: 160,
        preloader: true,
        fixedContentPos: false,
        gallery: {
            enabled: true // Aktifkan gallery jika ada banyak gambar
        },
        image: {
            verticalFit: true // Pastikan gambar fit di layar
        }
    });
}
...
menuItems.forEach(item => {
    ...
    links.forEach(link => {
        ...
        if (link.href === currentUrl) {
            // Check if the parent 'li' is inside an 'ul' with the class 'navigation'
            if (item.closest('ul.navigation')) {
...
// Also check other 'li' elements in the navigation that are not dropdowns
const navItems = document.querySelectorAll('ul.navigation > li > a');
...
// Select all 'a' elements inside the sidebar services list
const sidebarLinks = document.querySelectorAll('.sidebar__services-list li a');
...
// ============================
// ukuran font untuk judul gambarr
// ============================
